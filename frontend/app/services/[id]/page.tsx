          ...
          <div className="border-b border-slate-200 pb-10 mb-10 relative z-10">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 uppercase tracking-tight">
                ...
                <div className="grid grid-cols-3 gap-2 w-full md:max-w-[560px]">
                   ...
                   {hasCallablePhone ? (
                     <a href={`tel:${normalizedPhone}`} className="justify-center px-3 sm:px-4 py-2.5 border-2 border-[#1e293b] text-[#1e293b] font-semibold hover:bg-slate-50 transition-all flex items-center gap-1.5 text-xs sm:text-sm whitespace-nowrap bg-white" title={`Call ${rawContactPhone}`}>
                         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                         Call Now
                     </a>
                   ) : (
                     <button type="button" onClick={handleCopyPhone} className="justify-center px-3 sm:px-4 py-2.5 border-2 border-slate-300 text-slate-500 font-semibold transition-all flex items-center gap-1.5 text-xs sm:text-sm whitespace-nowrap bg-slate-50">
                       <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                       Copy Number
                     </button>
                   )}
                ...
                <div className="space-y-6">
                   {reviews.length > 0 ? visibleReviews.map((rev: any, i: number) => (
                     <div key={i} className="bg-white border border-slate-200 overflow-hidden">
                        <div className="bg-slate-100 p-5 flex items-center justify-between gap-4">
                          <div className="flex items-center gap-4 min-w-0">
                            <span className="text-6xl leading-none font-medium text-slate-800">{Number(rev.rating || 0).toFixed(1)}</span>
                            <div className="min-w-0">
                              <h4 className="font-semibold text-4xl md:text-2xl text-slate-800 truncate">{rev.user_name || 'User'}</h4>
                              <p className="text-slate-700 text-xl md:text-base font-medium">
                                {new Date(rev.created_at || Date.now())
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-slate-800 text-2xl md:text-base font-medium">Rating {Number(rev.rating || 0).toFixed(1)}</p>
                            <div className="flex text-amber-400 justify-end mt-1">
                              {[...Array(5)].map((_, s) => (
                                <svg key={s} className={`w-7 h-7 md:w-5 md:h-5 ${s < Math.round(rev.rating || 0) ? 'fill-current' : 'fill-slate-300'}`} viewBox="0 0 20 20"><path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/></svg>
                              ))}
                            </div>
                          </div>
                        </div>
                        <div className="px-5 py-8 border-t border-slate-200">
                          <p className="text-slate-800 text-2xl md:text-4xl lg:text-3xl xl:text-2xl leading-relaxed">{rev.comment}</p>
                        </div>
